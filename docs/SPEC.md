...
- **Tab capabilities (v1.37.0):** each Budget tab declares whether the month is its frame of reference and whether it has a "new" action. Month navigation (arrows, month label, "current month") therefore appears as a whole or not at all — it shows on the Budget and Plan tabs and is absent on Accounts, Subscriptions, Loans, Statistics and Split Expenses. The floating action button follows the same table: it creates the item that belongs to the active tab (entry, account, budget, subscription, loan, shared expense) and is hidden on Statistics, which has no create action.
