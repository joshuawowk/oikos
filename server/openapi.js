...
function buildPaths() {
  return {
    ...
    '/api/v1/preferences': {
      ...
      put: op({ summary: 'Update user preferences', tag: 'Preferences', description: 'Weather fields (`weather_provider`, `weather_lat`, `weather_lon`, `weather_city`, `weather_units`) and holiday fields (`holiday_country`, `holiday_subdivision`, `holiday_show_public`, `holiday_show_school`, `holiday_public_color`, `holiday_school_color`) are admin-only. `weather_lat`/`weather_lon` are validated to ±90 / ±180; colors must be 6-digit hex. Module feature toggles `health_cycle_enabled` (show the health cycle tab) and `rewards_require_approval` (redemptions need admin approval) are admin-only booleans, both default `true`.', stateChanging: true, requestBody: jsonBody(null) }),
    },
